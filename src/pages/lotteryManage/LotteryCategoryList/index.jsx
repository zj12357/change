...
@connect(({ LotteryCategoryListModel, loading, login }) => ({
  LotteryCategoryListModel,
  login,
  loading: loading.models.LotteryCategoryListModel,
}))
class TableList extends Component {
  ...
  columns = [
    ...
    },
    {
      ...
      dataIndex: 'img',
      render: text =>
        text ? <img src={filterImages(text)} alt="" style={{ width: '36px' }} /> : '',
    },
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'LotteryCategoryListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    if (!fields || (Array.isArray(fields) && fields.length <= 0)) {
      message.error('至少请选择一个彩钟');
      return false;
    }
    dispatch({
      type: 'LotteryCategoryListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'LotteryCategoryListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'LotteryCategoryListModel/remove',
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'LotteryCategoryListModel/add',
      payload: {
        ...fields,
      },
      callback: () => {
  ...
  render() {
    ...
    const { allResourcesList, yelResourcesList } = data; //分配彩钟列表
    const parentMethods = {
