...
@connect(({ LotteryRecordListModel, loading, login }) => ({
  LotteryRecordListModel,
  login,
  loading: loading.models.LotteryRecordListModel,
}))
class TableList extends Component {
  ...
  columns = [
    ...
    },
    {
      title: '红单率(%)',
      dataIndex: 'accuracy',
      render: val => val && val * 100 + '%',
    },
    {
      title: '连续命中数',
      dataIndex: 'straightHit',
    },
    {
      title: '总推荐量',
      dataIndex: 'recommendTotal',
    },
  ];
  ...
  getReloadList = params => {
    ...
    dispatch({
      type: 'LotteryRecordListModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...formValues,
  ...
  getStatusList = () => {
    ...
    dispatch({
      ...
      callback: data => {
        const list = data.lotteryTypeList || [];
        this.setState({ dictionaryList: list });
      },
    });
  };
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'LotteryRecordListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'LotteryRecordListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'LotteryRecordListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'LotteryRecordListModel/remove',
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'LotteryRecordListModel/add',
      payload: {
        ...fields,
      },
      callback: () => {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          ...
          <Col md={7} sm={24}>
            <FormItem label="玩法">
              {getFieldDecorator('playMethod')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <a
            style={{
