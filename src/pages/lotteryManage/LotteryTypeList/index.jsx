...
import { filtEleBtn, filterImages, takeIcon } from '@/utils/utils.js';
...
@connect(({ LotteryTypeListModel, loading, login }) => ({
  LotteryTypeListModel,
  login,
  loading: loading.models.LotteryTypeListModel,
}))
class TableList extends Component {
  ...
  columns = [
    ...
    },
    {
      ...
      dataIndex: 'img',
      render: text =>
        text ? <img src={filterImages(text)} alt="" style={{ width: '50px' }} /> : '',
    },
    {
      title: '排序',
      dataIndex: 'sort',
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'LotteryTypeListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    if (!fields || (Array.isArray(fields) && fields.length <= 0)) {
      message.error('至少请选择一个彩种分类');
      return false;
    }
    dispatch({
      type: 'LotteryTypeListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'LotteryTypeListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'LotteryTypeListModel/remove',
  ...
  render() {
    ...
    const { allResourcesList, yelResourcesList } = data; //分配彩种分类列表
    const parentMethods = {
