...
@connect(({ ParameterListModel, loading, login }) => ({
  ParameterListModel,
  login,
  loading: loading.models.ParameterListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'ParameterListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'ParameterListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'ParameterListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'ParameterListModel/remove',
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          <Col md={7} sm={24}>
            <FormItem label="域名参数">
              {getFieldDecorator('parameter')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
        </Row>
      </Form>
