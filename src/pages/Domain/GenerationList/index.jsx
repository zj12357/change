...
@connect(({ GenerationListModel, loading, login }) => ({
  GenerationListModel,
  login,
  loading: loading.models.GenerationListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'GenerationListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'GenerationListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'GenerationListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'GenerationListModel/remove',
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          ...
          <Col md={7} sm={24}>
            <FormItem label="短域名">
              {getFieldDecorator('sName')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
        </Row>
      </Form>
