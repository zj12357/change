...
@connect(({ DomainListModel, loading, login }) => ({
  DomainListModel,
  login,
  loading: loading.models.DomainListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'DomainListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'DomainListModel/enableAccount',
      payload: {
        id,
