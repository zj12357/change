...
@connect(({ BackgroundListModel, loading, login }) => ({
  BackgroundListModel,
  login,
  loading: loading.models.BackgroundListModel,
}))
class TableList extends Component {
  ...
  getReloadList = params => {
    ...
    dispatch({
      type: 'BackgroundListModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...formValues,
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'BackgroundListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'BackgroundListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'BackgroundListModel/remove',
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper>
        ...
        >
          {pathname.indexOf('BackgroundList') > -1 ? '新增背景图' : ''}
        </span>
        <Card bordered={false}>
