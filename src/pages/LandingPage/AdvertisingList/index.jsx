...
@connect(({ AdvertisingListModel, loading, login }) => ({
  AdvertisingListModel,
  login,
  loading: loading.models.AdvertisingListModel,
}))
class TableList extends Component {
  ...
  getReloadList = params => {
    ...
    dispatch({
      type: 'AdvertisingListModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...formValues,
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'AdvertisingListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'AdvertisingListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'AdvertisingListModel/remove',
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper>
        ...
        >
          {pathname.indexOf('AdvertisingLis') > -1 ? '新增广告图' : ''}
        </span>
        <Card bordered={false}>
