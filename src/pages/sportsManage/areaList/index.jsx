...
@connect(({ areaListModel, loading, login }) => ({
  areaListModel,
  login,
  loading: loading.models.areaListModel,
}))
class TableList extends Component {
  ...
  getStatusList = () => {
    ...
    dispatch({
      ...
      callback: data => {
        ...
        let AreaType = list.filter(item => item.parentCode === 'AreaType');
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'areaListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    if (!fields || (Array.isArray(fields) && fields.length <= 0)) {
      message.error('至少请选择一个区域');
      return false;
    }
    dispatch({
      type: 'areaListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'areaListModel/enableAccount',
      payload: {
        id,
          ...
          <Col md={8} sm={24}>
            <FormItem label="区域级别">
              {getFieldDecorator('level')(
                ...
                >
                  {
                    AreaType.map((item,index)=> <Option value={Number(item.code)} key={index}>{item.displayName}</Option>)
                  }
                </Select>,
              )}
            </FormItem>
  ...
  render() {
    ...
    const { allResourcesList, yelResourcesList } = data; //分配区域列表
    const parentMethods = {
