...
@connect(({ teamListModel, loading, login }) => ({
  teamListModel,
  login,
  loading: loading.models.teamListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'teamListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    if (!fields || (Array.isArray(fields) && fields.length <= 0)) {
      message.error('至少请选择一个球队');
      return false;
    }
    dispatch({
      type: 'teamListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'teamListModel/enableAccount',
      payload: {
        id,
  ...
  render() {
    ...
    const { allResourcesList, yelResourcesList } = data; //分配球队列表
    const parentMethods = {
