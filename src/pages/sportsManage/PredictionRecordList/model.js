import { addRule, getList, removeRule,ModifySysRole  , enableAccounts,CompetitionList} from './service';
import { message } from 'antd';
import { keysID, listId } from './keys.js';
const Model = {
  namespace: 'PredictionRecordListModel',
  state: {
    data: {
      list: [],
      pagination: {},
  ...
  effects: {
    *getCustList({ callback, payload }, { call, put }) {
      const response = yield call(CompetitionList, payload);
      if (response && response.resultData) {
        const { page = {} } = response.resultData;
        const lists = response.resultData.list || [];
