...
@connect(({ getUserStatisticsList, loading, login }) => ({
  getUserStatisticsList,
  login,
  loading: loading.models.getUserStatisticsList,
}))
class TableList extends Component {
  ...
  getReloadList = params => {
    const {
      dispatch,
      getUserStatisticsList: { data },
    } = this.props;
    const { pagination = {} } = data || {};
    const { current = 1 } = pagination;
    // console.log('pagination``````````````````',pagination)
    dispatch({
      type: 'getUserStatisticsList/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...params,
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'getUserStatisticsList/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'getUserStatisticsList/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'getUserStatisticsList/remove',
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'getUserStatisticsList/add',
      payload: {
        ...fields,
      },
      callback: () => {
  ...
  getVipLevelList = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'getUserStatisticsList/vipLevel',
      payload: {},
    });
  };
  getOftenGames = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'getUserStatisticsList/oftenGames',
      payload: {},
    });
  };
  getProxyLineList = val => {
    const { dispatch } = this.props;
    dispatch({
      type: 'getUserStatisticsList/proxyLine',
      payload: {
        name: val,
      },
    });
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          ...
          <Col md={16} sm={24}>
            <FormItem {...formItemLayout} label="类别规则">
              {getFieldDecorator('CategoryRules', {
                rules: [
                  {
                    required: false,
                    message: '请选择类别规则',
        ...
        >
          {rangeTextList.includes('当月存款') && (
            <div>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('deposit.min', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="当月存款大于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('deposit.max', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="当月存款小于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
            </div>
          )}
          {rangeTextList.includes('当月流水') && (
            <div>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('bet.min', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="当月流水大于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('bet.max', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="当月流水小于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
            </div>
          )}

          {rangeTextList.includes('累计存款') && (
            <div>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('depositTotal.min', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="累计存款大于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('depositTotal.max', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="累计存款小于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
            </div>
          )}
          {rangeTextList.includes('累计流水') && (
            <div>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('betTotal.min', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="累计流水大于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('betTotal.max', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="累计流水小于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
            </div>
          )}
          {rangeTextList.includes('复存次数') && (
            <div>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('depositCount.min', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="复存次数大于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('depositCount.max', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="复存次数小于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
            </div>
          )}
          {rangeTextList.includes('首存金额') && (
            <div>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('firstDeposit.min', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="首存金额大于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
              <Col md={6} sm={24}>
                <FormItem {...formItemLayout}>
                  {getFieldDecorator('firstDeposit.max', {
                    rules: [
                      {
                        required: false,
                        message: '请输入查询范围',
                  ...
                  })(<InputNumber placeholder="首存金额小于" style={{ width: '100%' }} />)}
                </FormItem>
              </Col>
            </div>
          )}
        ...
        >
          <Col md={11} sm={24}>
            <FormItem {...formItemLayout} label="上次存款时间">
          ...
          <Col md={11} sm={24}>
            <FormItem {...formItemLayout} label="上次联机时间">
        ...
        >
          <Col md={11} sm={24}>
            <FormItem {...formItemLayout} label="上次申请时间">
              {getFieldDecorator('lastApplyTime', {
                rules: [
                  {
                    required: false,
                    message: '请选择上次申请时间',
