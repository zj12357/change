...
@connect(({ GetFirstDespositListModel, loading, login }) => ({
  GetFirstDespositListModel,
  login,
  loading: loading.models.GetFirstDespositListModel,
}))
class TableList extends Component {
  state = {
    ...
    proxyLineList: [],
    ProxyLineGroup: [],
    despositDate: moment(moment().format('YYYY-MM-DD HH:mm:ss'), 'YYYY-MM-DD HH:mm:ss')
      .utc(8)
      .format(),
  };
  ...
  getReloadList = params => {
    const {
      dispatch,
      GetFirstDespositListModel: { data },
    } = this.props;
    ...
    dispatch({
      type: 'GetFirstDespositListModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...formValues,
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'GetFirstDespositListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'GetFirstDespositListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'GetFirstDespositListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'GetFirstDespositListModel/remove',
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'GetFirstDespositListModel/add',
      payload: {
        ...fields,
      },
      callback: () => {
