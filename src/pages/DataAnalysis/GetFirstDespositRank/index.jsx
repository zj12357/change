...
@connect(({ GetFirstDespositRankModel, loading, login }) => ({
  GetFirstDespositRankModel,
  login,
  loading: loading.models.GetFirstDespositRankModel,
}))
class TableList extends Component {
  ...
  getReloadList = params => {
    const {
      dispatch,
      GetFirstDespositRankModel: { data },
    } = this.props;
    ...
    dispatch({
      type: 'GetFirstDespositRankModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...formValues,
  ...
  getStatusList = () => {
    ...
    dispatch({
      ...
      callback: data => {
        ...
        let ContactStatus = list.filter(item => item.parentCode === 'ContactStatus');
        this.setState({ ProxyLineGroup, ContactStatus });
      },
    });
  };
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'GetFirstDespositRankModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'GetFirstDespositRankModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'GetFirstDespositRankModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'GetFirstDespositRankModel/remove',
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'GetFirstDespositRankModel/add',
      payload: {
        ...fields,
      },
      callback: () => {
  ...
  renderAdvancedForm() {
    const { form } = this.props;
    const { getFieldDecorator } = form;
    const { ProxyLineGroup, proxyLineList, ContactStatus } = this.state;
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row
          gutter={{
          ...
          <Col md={7} sm={24}>
            <FormItem label="最小金额">
              {getFieldDecorator('minAmt')(
                <InputNumber placeholder="请输入最小金额" style={{ width: '100%' }} />,
              )}
            </FormItem>
          </Col>
          <Col md={7} sm={24}>
            <FormItem label="最大金额">
              {getFieldDecorator('maxAmt')(
                <InputNumber placeholder="请输入最大金额" style={{ width: '100%' }} />,
              )}
            </FormItem>
          </Col>
        </Row>
          ...
          <Col md={7} sm={24}>
            <FormItem label="联系方式">
              {getFieldDecorator('isContact')(
                <Select style={{ width: '100%' }} placeholder="请选择是否有联系方式">
