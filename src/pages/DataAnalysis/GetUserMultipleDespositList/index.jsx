...
@connect(({ GetUserMultipleDespositListModel, loading, login }) => ({
  GetUserMultipleDespositListModel,
  login,
  loading: loading.models.GetUserMultipleDespositListModel,
}))
class TableList extends Component {
  state = {
    ...
    ProxyLineGroup: [],
    plid: [],
    onLineDate: moment(moment().format('YYYY-MM-DD HH:mm:ss'), 'YYYY-MM-DD HH:mm:ss')
      .utc(8)
      .format(),
  };
  ...
  getReloadList = params => {
    const {
      dispatch,
      GetUserMultipleDespositListModel: { data },
    } = this.props;
    ...
    dispatch({
      type: 'GetUserMultipleDespositListModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...formValues,
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'GetUserMultipleDespositListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'GetUserMultipleDespositListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'GetUserMultipleDespositListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'GetUserMultipleDespositListModel/remove',
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'GetUserMultipleDespositListModel/add',
      payload: {
        ...fields,
      },
      callback: () => {
  ...
  renderForm() {
    ...
    const { proxyLineList = [], ProxyLineGroup } = this.state;
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          ...
          <Col md={7} sm={24}>
            <FormItem label="用户复充日期">
              {getFieldDecorator('despositDate', {
                rules: [
                  {
                    required: false,
                    message: '请选择日期',
