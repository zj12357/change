...
@connect(({ GetBannerListModel, loading, login }) => ({
  GetBannerListModel,
  login,
  loading: loading.models.GetBannerListModel,
}))
class TableList extends Component {
  ...
  columns = [
    ...
    },
    {
      ...
      render: val => this.filterRediceName(val),
    },
    {
      title: '创建人',
  ...
  filterRediceName = did => {
    const { RedirectTypeList = [] } = this.state;
    const filtList = RedirectTypeList.filter(item => item.code === did);
    if (filtList.length > 0) {
      return filtList[0] && filtList[0].displayName;
    }
    return '';
  };
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'GetBannerListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'GetBannerListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'GetBannerListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'GetBannerListModel/remove',
