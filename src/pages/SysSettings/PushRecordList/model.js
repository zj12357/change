import { addRule, getList, removeRule,ModifySysRole  , 
  enableAccounts,SendPush,AllSendPush,RunSportPush,RunLotterPush,
  SendPushJG,DeletePush} from './service';
import { message } from 'antd';
import { keysID, listId } from './keys.js';

const typeName = {
  SendPush: SendPush,
  AllSendPush: SendPush,
  RunSportPush: RunSportPush,
  RunLotterPush: RunLotterPush,
}


const Model = {
  namespace: 'PushRecordListModel',
  state: {
    data: {
      list: [],
      pagination: {},
  ...
  effects: {
    ...
    *add({ payload, callback }, { call, put }) {
      const response = yield call( typeName[payload.type], payload);
      if (response && response.resultData) {
    ...
    *pushJG({ payload, callback }, { call, put }) {
      const response = yield call( payload.type === 'push' ? SendPushJG : DeletePush, {id:payload.id});
      if (response && response.resultData) {
        if (callback) callback();
      }
    },
