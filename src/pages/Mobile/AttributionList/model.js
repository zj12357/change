import { addMobile, getList, removeMobile, ImportMobile, MobileSupplyList } from './service';

import { keysID, listId } from './keys.js';
const Model = {
  namespace: 'AttributionListModel',
  state: {
    data: {
      list: [],
      roleList: [],
  ...
  effects: {
    ...
    *mobileSupply({ payload, callback }, { call, put }) {
      const response = yield call(MobileSupplyList, payload);
      if (response && response.resultData) {
        if (callback) callback(response.resultData);
      }
    },
