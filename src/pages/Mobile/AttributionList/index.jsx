...
@connect(({ AttributionListModel, loading, login }) => ({
  AttributionListModel,
  login,
  loading: loading.models.AttributionListModel,
}))
class TableList extends Component {
  ...
  getReloadList = params => {
    ...
    dispatch({
      type: 'AttributionListModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...formValues,
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'AttributionListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'AttributionListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'AttributionListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'AttributionListModel/remove',
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          <Col md={7} sm={24}>
            <FormItem label="省份">
              {getFieldDecorator('province')(<Input placeholder="请输入省份" />)}
            </FormItem>
          </Col>
          <Col md={7} sm={24}>
            <FormItem label="城市">
          ...
          <Col md={7} sm={24}>
            <FormItem label="归属地号码">
              {getFieldDecorator('code')(<Input placeholder="请输入归属地号码" />)}
            </FormItem>
          </Col>
        </Row>
      </Form>
