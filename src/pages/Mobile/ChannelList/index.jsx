...
@connect(({ ChannelListModel, loading, login }) => ({
  ChannelListModel,
  login,
  loading: loading.models.ChannelListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'ChannelListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'ChannelListModel/enableAccount',
      payload: {
        id,
