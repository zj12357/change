...
@connect(({ MobileListModel, loading, login }) => ({
  MobileListModel,
  login,
  loading: loading.models.MobileListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'MobileListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'MobileListModel/enableAccount',
      payload: {
        id,
  ...
  downloadFile = () => {
    const { dispatch } = this.props;
    dispatch({
      type: 'MobileListModel/ExportMobiles',
      payload: {},
      callback: data => {
        console.log('导出数据---', data);
        let link = document.createElement('a');
        link.setAttribute('download', '');
        // http://property.51yijiazhizhu.com:80/property-admin/bill/downloadTemplate
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          <Col md={7} sm={24}>
            <FormItem label="批次">
              {getFieldDecorator('batchNo')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={7} sm={24}>
            <FormItem label="手机号码">
  ...
  handleModalVisibleUpload = () => {
    this.setState({ modalVisibleUpload: !this.state.modalVisibleUpload });
  };
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper>
          ...
          <div className={styles.tableList}>
            ...
            <div className={styles.tableListOperator}>
              ...
              {filtEleBtn(
                ...
                menuText,
                buttonKey.ImportMobile,
              ) ? (
                <Button
                  type="primary"
                  icon={
                    takeIcon(this.props.login.dictionaryList, buttonKey.ImportMobile) || 'import'
                  }
                  onClick={() => this.handleModalVisibleUpload(true, 'add')}
                >
                  导入
                </Button>
              ) : (
              ...
              {filtEleBtn(
                this.props.login.sysUserPermissionList,
                menuText,
                buttonKey.ExportMobile,
              ) ? (
                <Button
                  type="primary"
                  icon={
                    takeIcon(this.props.login.dictionaryList, buttonKey.ExportMobile) || 'export'
                  }
        ...
        <UpdateForm modalVisible={modalVisibleUpload} {...parentMethodsUpload} />
      </PageHeaderWrapper>
    );
  }
}
