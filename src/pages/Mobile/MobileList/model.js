...
const Model = {
  ...
  effects: {
    ...
    *ExportMobiles({ payload, callback }, { call, put }) {
      const response = yield call(ExportMobile, payload);
      if (response && response.resultData) {
        if (callback) callback(response.resultData);
      }
    },
    ...
    *getLotMobile({ payload, callback }, { call, put }) {
      const response = yield call(LotMobile, payload);
      if (response && response.resultData) {
        if (callback) callback(response.resultData);
      }
    },
