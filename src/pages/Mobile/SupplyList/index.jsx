...
@connect(({ SupplyListModel, loading, login }) => ({
  SupplyListModel,
  login,
  loading: loading.models.SupplyListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'SupplyListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'SupplyListModel/enableAccount',
      payload: {
        id,
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          <Col md={7} sm={24}>
            <FormItem label="供应商">
              {getFieldDecorator('desc')(<Input placeholder="请输入供应商" />)}
            </FormItem>
          </Col>
        </Row>
      </Form>
