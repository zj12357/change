...
const typeMethods = {
  AddCharRoom: addRule,
  AddMembers: RegisterUser,
  RegisterAdminUser: RegisterAdminUser
}

const Model = {
  namespace: 'charRoomListModel',
  state: {
    data: {
      list: [],
      pagination: {},
  ...
  effects: {
    ...
    *add({ payload, callback }, { call, put }) {
      const response = yield call( typeMethods[payload.addType], payload);
      if (response && response.resultData && response.resultData !== -1) {
