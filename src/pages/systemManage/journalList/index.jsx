...
@connect(({ journalListModel, loading, login }) => ({
  journalListModel,
  login,
  loading: loading.models.journalListModel,
}))
class TableList extends Component {
  ...
  getReloadList = params => {
    const {
      dispatch,
      journalListModel: { data },
    } = this.props;
    const { pagination = {} } = data || {};
    const { current = 1 } = pagination;

    dispatch({
      type: 'journalListModel/fetch',
      payload: {
        pageIndex: current,
        pageSize: 10,
        ...params,
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'journalListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'journalListModel/enableAccount',
      payload: {
        id,
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          ...
          <Col md={7} sm={24}>
            <FormItem label="登录IP">
              {getFieldDecorator('loginIP')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={7} sm={24}>
            <FormItem label="时间段">
