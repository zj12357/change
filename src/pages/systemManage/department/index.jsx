...
@connect(({ departmentModel, loading, login }) => ({
  departmentModel,
  login,
  loading: loading.models.departmentModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisible = record => {
    ...
    this.setState({ editRoleDate: record });
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'departmentModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    if (!fields || (Array.isArray(fields) && fields.length <= 0)) {
      message.error('至少请选择一个部门');
      return false;
    }
    dispatch({
      type: 'departmentModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'departmentModel/enableAccount',
      payload: {
        id,
  ...
  handleModalVisible = (flag, type, parentID) => {
    ...
    if (type === 'small') {
      ...
      parentID = selectedRows[0].deptID;
    }
  ...
  render() {
    ...
    const { allResourcesList, yelResourcesList } = data; //分配部门列表
    const parentMethods = {
