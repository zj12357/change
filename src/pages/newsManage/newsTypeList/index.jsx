...
@connect(({ newsTypeListModel, loading, login }) => ({
  newsTypeListModel,
  login,
  loading: loading.models.newsTypeListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'newsTypeListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'newsTypeListModel/enableAccount',
      payload: {
        id,
