import { Button, Card, Col, Form, Input, Row, Select, Popover, message, Modal } from 'antd';
...
@connect(({ InformationListModel, loading, login }) => ({
  InformationListModel,
  login,
  loading: loading.models.InformationListModel,
}))
class TableList extends Component {
  ...
  getStatusList = () => {
    ...
    dispatch({
      ...
      callback: data => {
        ...
        let CommentStatus = list.filter(item => item.parentCode === 'CommentStatus');
        this.setState({ CommentStatus });
      },
    });
  };
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      dispatch({
        type: 'InformationListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'InformationListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'InformationListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'InformationListModel/remove',
