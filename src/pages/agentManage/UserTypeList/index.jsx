...
@connect(({ UserTypeListModel, loading, login }) => ({
  UserTypeListModel,
  login,
  loading: loading.models.UserTypeListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (flag) {
      dispatch({
        type: 'UserTypeListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'UserTypeListModel/enableAccount',
      payload: {
        id,
