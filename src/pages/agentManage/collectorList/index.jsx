...
@connect(({ collectorListModel, loading, login }) => ({
  collectorListModel,
  login,
  loading: loading.models.collectorListModel,
}))
class TableList extends Component {
  ...
  getStatusList = () => {
    ...
    dispatch({
      ...
      callback: data => {
        ...
        const purposeList = list.filter(item => item.parentCode === 'ProxyCollectionState'); //采集器状态
        const modeList = list.filter(item => item.parentCode === 'CollectType'); //采集类型
        const PDCollectionCategory = list.filter(
          item => item.parentCode === 'PDCollectionCategory',
        ); //采集器分类
        const purposeUsage = list.filter(item => item.parentCode === 'Usage'); //采集器用途
        this.setState({ modeList, purposeList, PDCollectionCategory, purposeUsage });
      },
    });
  };
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (!!flag) {
      // dispatch({
      //   type: 'collectorListModel/SysUserRoleLists',
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'collectorListModel/update',
      payload: fields,
      callback: () => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'collectorListModel/enableAccount',
      payload: {
        id,
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'collectorListModel/remove',
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          ...
          <Col md={7} sm={24}>
            <FormItem label="采集方式">
              {getFieldDecorator('collectTypeID', {
                rules: [
                  {
                    required: false,
                    message: '请选择',
  ...
  renderAdvancedForm() {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          <Col md={7} sm={24}>
            <FormItem label="采集器分类">
              {getFieldDecorator('collectionCategoryID', {
                rules: [
                  {
                    required: false,
                    message: '请选择',
