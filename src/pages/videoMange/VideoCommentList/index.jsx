import { Button, Card, Col, Form, Icon, Input, Row, Select, message, Popover, Modal } from 'antd';
...
@connect(({ VideoCommentListModel, loading, login }) => ({
  VideoCommentListModel,
  login,
  loading: loading.models.VideoCommentListModel,
}))
class TableList extends Component {
  ...
  enableAccount = (record, type = 'isShow') => {
    ...
    dispatch({
      type: 'VideoCommentListModel/enableAccount',
      payload: params,
      callback: () => {
        this.getReloadList();
      },
  ...
  getStatusList = () => {
    ...
    dispatch({
      ...
      callback: data => {
        ...
        let VideoCommentShowStatus = list.filter(
          item => item.parentCode === 'VideoCommentShowStatus',
        );
        this.setState({ VideoCommentShowStatus });
      },
    });
  };
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (flag) {
      dispatch({
        type: 'VideoCommentListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'VideoCommentListModel/update',
      payload: {
        role: id,
        res: fields,
      },
  ...
  handleMenuClick = (type, record) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        dispatch({
          type: 'VideoCommentListModel/remove',
  ...
  handleAdd = fields => {
    ...
    dispatch({
      type: 'VideoCommentListModel/add',
      payload: {
        ...fields,
      },
      callback: () => {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        >
          ...
          <Col md={7} sm={24}>
            <FormItem label="视频">
              {getFieldDecorator('videoById')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
  ...
  renderAdvancedForm() {
    ...
    const { VideoCommentShowStatus } = this.state;
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row
          gutter={{
            ...
            <FormItem label="是否显示">
              {getFieldDecorator('isShow')(
                <Select style={{ width: '100%' }} placeholder="请选择">
                  {VideoCommentShowStatus.map((item, index) => (
