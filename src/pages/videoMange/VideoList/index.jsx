...
@connect(({ VideoListModel, loading, login }) => ({
  VideoListModel,
  login,
  loading: loading.models.VideoListModel,
}))
class TableList extends Component {
  ...
  handleUpdateModalVisibleRole = (flag, record) => {
    ...
    if (flag) {
      dispatch({
        type: 'VideoListModel/getResourceByAdminIdServe',
        payload: record.id,
      }).then(() => {
  ...
  enableAccount = record => {
    ...
    const { id } = record;
    dispatch({
      type: 'VideoListModel/enableAccount',
      payload: {
        id,
